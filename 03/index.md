...
```
Хочу напомнить, что в отличае от Hakell, здесь отступы не играют никакой роли кроме декоративной и код отфарматирован так только что бы его было легче воспринимать. Вы вольны форматировать его так, как вам будет удобно. 
Здесь выражения `rule_1` и `rule_2` - это одно или несколько условий. В случае успешного выполнения условия, будет выполнен блок кода `Expression` (может содержать одну или несколько комманд) идущий за ним. Таких логических ветвей может быть сколько угодно много.

Обратите внимание на последний блок `true -> "You're very fat.`. Что это за условие? Это альтернатива оператору `else`. Блок кода идущий за ним будет выполнен если ни одно уловие не пройдет проверку. Важно запомнить, что этот оператор обязателен. Ведь, как мы помним, в Erlang любое выражение должно возвращать результат. Если не описать этот блок, то модуль скомпилируется, но когда поток выполнения проверит все условия и не обнаружит блок `true`, будет сгенерированно исключение.
```erlang
...
Помимо `if` в Erlang есть еще один оператор ветвления - `case of`. И этот оператор подобен целой функции вставленой в другую функцию. Он позволяет делать выбор не только на основании условия, но так же дает возможнотсь использовать сопоставления с образцом и охранные выражения. В общем виде он выглядит так:
```erlang
case Rule of
    Val_1 -> Expression;
    Val_2 -> Expression
...
```
Здесь `Rule` - это переменная или выражение, результат гкоторого будет проверяться. Дальше следуют уже знакомые нам блоки "условие - выражение" (`Val_1 -> Expression;`). В условии можно использовать сопоставления с образцом и охранные выражения, что делает эту конструкцию невероятно гибкой.
...
```
При выполнении этой функции наш кортеж будет сопоставлен с переменной `Temp`, затем он будет найден паттерн, под который этот кортеж подойдет. Затем будет проведена проверка охранными выражениями и если она будет пройдена - функция вернет соответствующую строку.
Здесь мы так же как и раньше добавляем в конец выражение, которое будет перехватывать все неподошедшие варианты.

Как мы видим, выражение `case of` почти полностью можно переместить в объявление функции. Так когда же его стоит использовать? Ответ прост: как вам больше нравиться. Различия между этими двумя конструкциями минимальны. Поэтому используйте тот вариант, который вам легче читать.

Заключение
----------

В этой главе мы то, как можно управлять потоком выполнения команд внутри функций. Узанли, какие для этого существют конструкции и как их использовать. Так же мы познакомились с охранными выражениями.

В следующей главе мы поближе познакомимся с системой типов Erlang.
